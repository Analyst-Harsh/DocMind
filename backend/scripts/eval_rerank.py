"""
Compare hybrid (dense+BM25) retrieval vs hybrid+cross-encoder-rerank for
recursive chunking + text-embedding-3-small against the golden query set.

Usage:
  python -m scripts.eval_rerank
  python -m scripts.eval_rerank --k 5 --verbose
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compare hybrid vs hybrid+rerank retrieval."
    )
    ...
    if hybrid_collection not in existing:
        ...
        report = run_strategy(
            f"{STRATEGY} (hybrid+rerank)", reranked_fn, golden, args.k
        )
